...
class AdController extends Controller
{
    ...
    public function create()
    {
        ...
        if (empty($user->name) || empty($user->description)) {
            // Если нет, перенаправляем на страницу редактирования профиля
            return redirect()->route('profile.edit');
        }

        // Если профиль заполнен, разрешаем создание объявления
    ...
    public function store(Request $request)
{
    ...
    $ad->save(); // Сохраняем объявление в базу данных, только после этого будет доступен ID
...
public function myAds()
{
    ...
    // Передаем и объявления, и пользователя в представление
